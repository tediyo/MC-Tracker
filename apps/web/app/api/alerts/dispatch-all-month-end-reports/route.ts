...
export async function POST(req: Request) {
  try {
    ...
    for (const user of usersToProcess) {
      ...
      // 5. Dispatch email via Brevo HTTPS API (Port 443) or fallback to Gmail SMTP
      ...
      if (brevoApiKey) {
        ...
        const brevoFromEmail = emailMatch ? emailMatch[1].trim() : rawBrevoFrom.trim();
        const brevoFromName = process.env.BREVO_FROM_NAME || (nameMatch ? nameMatch[1].trim() : "MC Tracker");
        ...
        try {
          ...
          if (res.ok) {
            console.log(`[Batch Email] Dispatched to ${email} via Brevo (Message ID: ${brevoData.messageId})`);
            results.sent.push({
              email,
              totalIncome,
              totalCosts,
            ...
            console.error(`[Batch Email] Brevo send failed for ${email}, falling back to SMTP:`, brevoData);
          }
        } catch (brevoErr: any) {
          console.error(`[Batch Email] Brevo network error for ${email}, falling back to SMTP:`, brevoErr.message);
        }
      }
      ...
      if (!sentViaBrevo) {
        ...
        console.log(`[Batch Email] Dispatched to ${email} via SMTP (Message ID: ${info.messageId})`);
        results.sent.push({
          email,
          totalIncome,
          totalCosts,
