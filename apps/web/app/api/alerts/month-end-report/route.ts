...
export async function POST(req: Request) {
  try {
    ...
    // 1. Dispatch via Brevo HTTPS API (Port 443 - no port blocks on Render/Vercel)
    ...
    if (brevoApiKey) {
      ...
      try {
        ...
        if (res.ok) {
          console.log("[Brevo] Month-End Email with PDF dispatched successfully:", brevoData.messageId);
          ...
          console.error("[Brevo] Failed to send Month-End report, falling back to SMTP:", brevoData);
        }
      } catch (brevoErr: any) {
        console.error("[Brevo] Network error, falling back to SMTP:", brevoErr.message);
      }
    }
    ...
    console.error("[Month-End Email Error]:", error);
